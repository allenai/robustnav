...
class RoboThorEnvironment:
    ...
    def __init__(self, all_metadata_available: bool = True, **kwargs):
        ...
        self._drift_deg = f(
            kwargs, "drift_deg", 1.15
        )  # Hardcoded default value if drift is set
        ...
        # Motion Bias (Constant Specifications)
    ...
    def reset(
        ...
        if scene_name is not None and scene_name != self.scene_name:
            ...
            if self._camera_crack:  # Add camera-crack if specified
                ...
                scene_split = (
                    scene_name.split("_")[-2][-1] + scene_name.split("_")[-1]
                )  # Environment crack seed is fixed based on the environment name
                crack_seed = int(scene_split)
                print("Camera-Crack applied")
