...
class OnPolicyRLEngine(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Check if action prediction mode is on
        # Set to false if off for further computations
        try:
        ...
        # Check if rotation prediction mode is on
        # Set to false if off for further computation
        try:
    ...
    def update(self, rollouts: RolloutStorage):
        ...
        for e in range(self.training_pipeline.update_repeats):
            ...
            for bit, batch in enumerate(data_generator):
                ...
                # Initialize aux task logits with None
                pred_action_logits, pred_rotation_logits = None, None

                if self.inv_mode:
                    # If in inverse dynamics mode
                    # obtain action-prediction logits
                    pred_action_logits = actor_critic_output.extras[
                        "pred_action_logits"
                    ]

                if self.rot_mode:
                    # If in rotation prediction mode
                    # obtain rotation-preciction logits
                ...
                for loss_name in self.training_pipeline.current_stage_losses:
                    ...
                    current_loss, current_info = loss.loss(
                        ...
                        pred_action_logits=pred_action_logits,  # Will add aux-task logits if present
                        pred_rotation_logits=pred_rotation_logits,  # Will add aux-task logits if present
                    )
